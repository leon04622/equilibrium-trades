...
/** Must match HL `approveBuilderFee` / order builder field (server validates substring). */
...
export function buildEquilibriumBuilderApprovalMessage(
  ...
  return [
    ...
    `Hyperliquid builder (approveBuilderFee): ${EQUILIBRIUM_HL_BUILDER_ADDRESS_CHECKSUM}`,
  ].join("\n");
}
