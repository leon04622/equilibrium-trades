...
export function WalletUsdcStrip({ className }: { className?: string }) {
  ...
  if (!connected || isLoadingWalletUsdc || walletUsdcArbitrum < 0.01) return null;

  return (
    <Link
      to="/funding?tab=deposit&activate=1"
      className={cn(
        "flex items-center justify-center gap-2 border-b border-emerald-500/30 bg-emerald-500/10 px-3 py-1.5 text-xs text-emerald-900 dark:text-emerald-200 hover:bg-emerald-500/15 transition-colors",
        className,
      )}
