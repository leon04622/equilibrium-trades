...
function formatJoined(u: WalletUser): string {
...
/**
 * Equilibrium Unified Admin Command Center — master-wallet signature session.
 * Tabs: CRM, Support Hub, Hyperliquid analytics (Apex Sovereign–style sovereign HUD).
 */
export default function UnifiedAdmin() {
  ...
  const handleSignIn = async () => {
    ...
    try {
      ...
      toast({ title: "Command Center unlocked", description: "Session is valid for several hours." });
    } catch (e: unknown) {
      toast({
        title: "Access denied",
  ...
  const grantProMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      toast({ title: "Pro granted", description: "Manual override on; no Stripe charge required." });
    },
    onError: (e: Error) =>
      toast({ title: "Error", description: e.message, variant: "destructive" }),
  });

  const grantMentoringMutation = useMutation({
    mutationFn: async (walletAddress: string) => {
      const enc = encodeURIComponent(walletAddress);
      const res = await fetch(`/api/admin-equilibrium/users/${enc}/subscription`, {
        method: "PATCH",
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["admin-equilibrium", "users"] });
      toast({
        title: "Mentoring granted",
        description: "Full tier access without the $500/mo Stripe plan.",
      });
    },
    onError: (e: Error) =>
      toast({ title: "Error", description: e.message, variant: "destructive" }),
      ...
      <div className="min-h-[70vh] flex items-center justify-center p-6">
        <Card className="max-w-md w-full border-primary/20">
          <CardHeader className="text-center">
            ...
            <CardTitle>Unified Admin Command Center</CardTitle>
            <CardDescription>
              Sign with your master admin wallet to open CRM, support, and platform analytics.
              Set <code className="text-xs">ADMIN_EQUILIBRIUM_MASTER_WALLET</code> on the server to pin
  ...
  return (
    <div className="p-4 md:p-6 max-w-7xl mx-auto space-y-4">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          <h1 className="text-2xl font-bold tracking-tight">Equilibrium Command Center</h1>
          <p className="text-sm text-muted-foreground">
            CRM · Support hub · Hyperliquid analytics (signature session)
          </p>
        </div>
        ...
        <TabsContent value="users">
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Wallets, emails, join dates, tiers. Grant Pro or Mentoring without Stripe.
              </CardDescription>
              <div className="pt-2">
              ...
              {usersQuery.isLoading ? (
                ...
                <ScrollArea className="h-[min(60vh,520px)]">
                  <Table>
                    ...
                    <TableBody>
                      {filteredUsers.map((u) => {
                        const proFullyGranted =
                          u.subscriptionTier === "pro" && u.subscriptionActive && u.manualProOverride;
                        const mentoringFullyGranted =
                          u.subscriptionTier === "mentoring" &&
                        ...
                        return (
                          <TableRow key={u.id}>
                            ...
                            <TableCell className="text-xs space-x-1">
                              {u.manualProOverride && (
                                <Badge className="bg-amber-600/90">Manual bypass</Badge>
                              )}
                              {u.builderCodeApproved && <Badge variant="secondary">Builder</Badge>}
                            </TableCell>
                            <TableCell className="text-xs text-muted-foreground">
                              {u.instantTradingCompletedAt
                                ? new Date(u.instantTradingCompletedAt as unknown as string).toLocaleString()
                                : "—"}
                            </TableCell>
                            <TableCell className="text-right space-x-1 whitespace-nowrap">
                              <Button
                                ...
                                variant="secondary"
                                className="h-8"
                                disabled={grantProMutation.isPending || proFullyGranted}
                              ...
                              <Button
                                ...
                                variant="outline"
                                className="h-8"
                                disabled={grantMentoringMutation.isPending || mentoringFullyGranted}
                                onClick={() => grantMentoringMutation.mutate(u.walletAddress)}
                              >
          ...
          <div className="grid md:grid-cols-2 gap-4">
            <Card>
              <CardHeader>
                ...
                <CardDescription>Real-time board — select a thread and reply.</CardDescription>
              </CardHeader>
