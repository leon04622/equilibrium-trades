  ...
  return (
    <div className="p-4 md:p-6 max-w-7xl mx-auto space-y-4">
      <div className="flex items-center gap-2">
        ...
        <div>
          <h1 className="text-xl font-bold">Equilibrium Admin & CRM</h1>
          <p className="text-xs text-muted-foreground">
            Data from <code className="text-[10px]">/api/users</code> ·{" "}
            <code className="text-[10px]">/api/messages</code> · Pro ${TIER_PRO} / Mentoring ${TIER_MENTOR}
          </p>
        </div>
      </div>
      ...
      <div className="flex flex-col md:flex-row gap-4">
        <aside className="w-full md:w-52 shrink-0 space-y-1 rounded-xl border bg-card p-2">
          {nav("users", "Users", <Users className="h-4 w-4" />)}
          {nav("support", "Support", <MessageSquare className="h-4 w-4" />)}
          {nav("l1", "L1 Analytics", <BarChart3 className="h-4 w-4" />)}
        </aside>
          ...
          {tab === "users" && (
            <Card>
              <CardHeader className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
                <div>
                  <CardTitle>Users & CRM</CardTitle>
                  <CardDescription>Wallet users from GET /api/users</CardDescription>
                </div>
              ...
              <Card>
                <CardHeader className="flex flex-row items-center justify-between">
                  <div>
                    ...
                    <CardDescription>GET /api/messages (newest threads first)</CardDescription>
                  </div>
