...
import type { TutorialVideo, SupportMessage, TradeJournalEntry, TradeJournalStats } from "@shared/schema";
...
export default function AdminCommandCenter() {
  ...
  const { data: crmJournalStats, isLoading: crmJournalStatsLoading } = useQuery({
    queryKey: ["crm-user-journal-stats", address, crmJournalWallet],
    enabled: !!address && !!crmJournalWallet,
    queryFn: async () => {
      const { data, status } = await api.get<TradeJournalStats>(
        `/api/trade-journal/stats/${encodeURIComponent(crmJournalWallet!)}`,
      );
      if (status === 401 || status === 403) throw new Error("Unauthorized");
  ...
  const { data: crmJournalEntries = [], isLoading: crmJournalEntriesLoading } = useQuery({
    queryKey: ["crm-user-journal-entries", address, crmJournalWallet],
    enabled: !!address && !!crmJournalWallet,
    queryFn: async () => {
      const { data, status } = await api.get<TradeJournalEntry[]>(
        `/api/trade-journal/entries/${encodeURIComponent(crmJournalWallet!)}?limit=30`,
      );
      if (status === 401 || status === 403) throw new Error("Unauthorized");
  ...
  return (
    <div className="p-4 md:p-8 max-w-6xl mx-auto space-y-6 pb-24 md:pb-8">
                          ...
                          <TableRow key={row.wallet}>
                            <TableCell className="font-mono text-xs align-top break-all max-w-[180px]">
                              <button
                                ...
                                onClick={() => setCrmJournalWallet(row.wallet)}
                              >
                                {row.wallet}
                              </button>
                            </TableCell>
      ...
      <Dialog open={crmJournalWallet != null} onOpenChange={(open) => !open && setCrmJournalWallet(null)}>
